...
DATASET_PRESET="${DATASET_PRESET:-reasoning_focus_v2}"
...
TS="$(date +%F_%H%M)"
SWEEP_NAME="d24_r32_adamw_partial_fr20_mixv2_seed_sweep_${TS}"
...
BASELINE_SUMMARY="${BASELINE_SUMMARY:-${NOTES_DIR}/d24_r32_adamw_partial_fr20_mixv2_2026-03-29_1040_s768_gc_best_s300_confirm_summary_1000_2026-03-29_1200.txt}"
...
write_decision() {
  python3 - "${RESULTS_TSV}" "${DECISION_MD}" "${BASELINE_SUMMARY}" <<'PY'
import csv
import math
import re
import sys
...
for r in full_rows:
    try:
        ...
        continue
    if gsm >= 4.0 and mmlu >= 27.0:
        threshold_rows.append(r)

if len(threshold_rows) >= 2:
    decision = "promote_mixv2_s768_main_branch"
    reason = "at least 2 seeds met the replication thresholds"
elif len(full_rows) >= 1:
    decision = "hold_mixv2_provisional"
    reason = "the winner exists, but the seed sweep did not show enough stable replications"
else:
    decision = "plan_mixv3_on_s768"
...
if not math.isnan(baseline_gsm):
    lines.append(f"- baseline_mixv2_gsm8k_pass8: `{baseline_gsm:.2f}%`")
if not math.isnan(baseline_mmlu):
    lines.append(f"- baseline_mixv2_mmlu: `{baseline_mmlu:.2f}%`")
if not math.isnan(baseline_spell):
    lines.append(f"- baseline_mixv2_spellingbee: `{baseline_spell:.2f}%`")
lines.append("")
...
log "[info] seeds=${SEEDS_CSV}"
log "[info] deterministic=${DETERMINISTIC} eval_seed=${EVAL_SEED}"
log "[info] baseline=${BASELINE_SUMMARY}"
log "[info] results=${RESULTS_TSV}"

IFS=',' read -r -a seeds <<< "${SEEDS_CSV}"
for seed in "${seeds[@]}"; do
  prefix="d24_r32_adamw_partial_fr20_mixv2_seed${seed}"
