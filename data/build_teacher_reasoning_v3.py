...
DEFAULT_OUT = Path(__file__).with_name("teacher_reasoning_v3.jsonl")
DEFAULT_META = Path(__file__).with_name("teacher_reasoning_v3_metadata.json")
...
MIN_REWARD = 0.18
MAGPIE_SCAN_LIMIT = 30000
OPENTHOUGHTS_SCAN_LIMIT = 114000
OPENR1_SCAN_LIMIT = 30000
...
def extract_answer_hint(text: str) -> Optional[str]:
    for pattern in ANSWER_PATTERNS:
        m = pattern.search(text)
        if m:
            value = normalize_text(m.group(1))
            value = value.strip(" .$")
            if value and re.search(r"[A-Za-z0-9]", value) and value not in {"=", "+", "-", "*", "/"}:
                return value
    return None
...
def normalize_teacher_answer(text: str) -> str:
    ...
    for raw in sentence_chunks(text):
        ...
        if not raw:
            continue
        if lowered in {"therefore", "thus", "hence"}:
            continue
        if raw in chunks:
            continue
        words = len(raw.split())
        if total_chars + len(raw) > 520:
            break
        if total_words + words > 120:
            break
        chunks.append(raw)
        total_chars += len(raw)
        total_words += words
    ...
    if answer_hint:
        cleaned = []
        for chunk in chunks:
            if answer_hint.lower() in chunk.lower():
                continue
            cleaned.append(chunk)
        explanation = " ".join(cleaned[:3]).strip()
        if explanation:
            return normalize_text(f"Answer: {answer_hint}\nWhy: {explanation}")
        return normalize_text(f"Answer: {answer_hint}")

    return normalize_text(" ".join(chunks[:4]))


def is_valid_pair(user: str, assistant: str) -> bool:
    user = normalize_user_prompt(user)
    assistant = normalize_teacher_answer(assistant)
...
def brevity_rank(user: str, assistant: str) -> int:
    ...
    return max(0, 900 - assistant_len) * 10_000 + max(0, 220 - assistant_words) * 1_000 + max(0, 700 - user_len)


def selection_score(seed: int, payload: str, user: str, assistant: str, reward: float = 0.0) -> int:
    tie = stable_score(seed, payload) % 1000
    base = brevity_rank(user, assistant) * 1000 + tie
    if reward > 0:
        base += int(reward * 1000) * 1_000_000_000
    return base


def push_topk(heap: List[Tuple[int, str, Candidate]], k: int, score: int, cand: Candidate) -> None:
    item = (score, cand.key, cand)
    if len(heap) < k:
...
def extract_solution_from_openthoughts(text: str) -> str:
    ...
    return normalize_teacher_answer(text)
...
def collect_magpie(seed: int, magpie_quotas: Dict[str, int]) -> Tuple[List[Candidate], Dict[str, int]]:
    ...
    heaps: Dict[str, List[Tuple[int, str, Candidate]]] = {cat: [] for cat in magpie_quotas}
    seen = set()
    scanned = 0
    ...
    for ex in ds:
        ...
        score = selection_score(seed, f"magpie:{category}:{user}", user, assistant, reward=reward)
        push_topk(heaps[category], magpie_quotas[category], score, cand)

    selected: List[Candidate] = []
    for heap in heaps.values():
        selected.extend(c for _, _, c in sorted(heap, key=lambda x: -x[0]))
...
def collect_openthoughts(seed: int, quotas: Dict[str, int]) -> Tuple[List[Candidate], Dict[str, int]]:
    ...
    heaps: Dict[str, List[Tuple[int, str, Candidate]]] = {cat: [] for cat in quotas}
    seen = set()
    scanned = 0
    ...
    for ex in ds:
        ...
        if conv[0].get("from") != "user" or conv[1].get("from") != "assistant":
            continue
        user = normalize_user_prompt(conv[0].get("value", ""))
        ...
        score = selection_score(seed, f"open-thoughts:{category}:{user}", user, assistant)
        push_topk(heaps[category], quotas[category], score, cand)
...
def collect_openr1(seed: int, target: int) -> Tuple[List[Candidate], Dict[str, int]]:
    ...
    heap: List[Tuple[int, str, Candidate]] = []
    seen = set()
    scanned = 0
    accepted = 0
    ...
    for ex in ds:
        ...
        if scanned > OPENR1_SCAN_LIMIT:
            break
        user = normalize_user_prompt(ex.get("problem", ""))
        assistant = normalize_teacher_answer(ex.get("solution", ""))
        if not is_valid_pair(user, assistant):
            continue
        dedupe_key = user.lower()
        if dedupe_key in seen:
        ...
        score = selection_score(seed, f"open-r1:{user}", user, assistant)
        push_topk(heap, target, score, cand)

    selected = [c for _, _, c in sorted(heap, key=lambda x: -x[0])]
