...
TS="$(date +%F_%H%M)"
SWEEP_TAG="d24_sft_recovery_${TS}"
...
run_trial() {
  ...
  log "START trial=${label} out=${out_tag} seq=${seq} total_batch=${total_batch} lora_lr=${lora_lr} gc=${use_gc}"
  ...
  log "EVAL trial=${label} out=${out_tag} step=${NUM_ITER} max_problems=${EVAL_MAX}"
  set +e
  "${HOME}/nanochat-learn/scripts/run_chat_eval_confirm_1k.sh" "${out_tag}/best" "${NUM_ITER}" "${EVAL_MAX}" |& tee "${eval_log}" | tee -a "${CHAIN_LOG}"
...
log "SWEEP start tag=${SWEEP_TAG} base=${BASE_MODEL_TAG}@${BASE_MODEL_STEP} num_iter=${NUM_ITER} eval_max=${EVAL_MAX}"

run_trial "s1024_lr5e5" 1024 8192 5e-5 0
run_trial "s1280_lr5e5" 1280 7680 5e-5 0
run_trial "s1536gc_lr3e5" 1536 7680 3e-5 1

best_line="$(tail -n +2 "${RESULTS_TSV}" | awk -F '\t' '$8 != "-1" {print $0}' | sort -t $'\t' -k8,8nr | head -n 1 || true)"

{
  echo "# SFT Recovery Sweep Summary"
  echo
  echo "- tag: \`${SWEEP_TAG}\`"
  ...
  if [ -n "${best_line}" ]; then
    ...
    echo '```bash'
    echo "~/nanochat-learn/scripts/run_chat_eval_confirm_1k.sh ${best_out}/best ${NUM_ITER} 1000"
    echo '```'
  else
    echo "## Winner"
    echo
