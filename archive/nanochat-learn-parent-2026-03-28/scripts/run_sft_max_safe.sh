...
TS="$(date +%F_%H%M)"
RUN_BASE="d18_clean200k_sftmax_${TS}"
OUT_BASE="${RUN_BASE}"
...
if run_sft 2048 8192 "${OUT_BASE}" "${RUN_BASE}" "${LOG_2048}"; then
  echo "[done] 2048 run finished successfully" | tee -a "${MASTER_LOG}"
  exit 0
fi

OOM_RE='out of memory|cuda out of memory|CUDNN_STATUS_ALLOC_FAILED|CUDA error: out of memory'
if rg -qi "${OOM_RE}" "${LOG_2048}"; then
  echo "[warn] OOM detected, retrying with --max-seq-len 1536" | tee -a "${MASTER_LOG}"
  RETRY_OUT="${OUT_BASE}_s1536"
  RETRY_RUN="${RUN_BASE}_s1536"
  run_sft 1536 7680 "${RETRY_OUT}" "${RETRY_RUN}" "${LOG_1536}"
  echo "[done] 1536 retry finished" | tee -a "${MASTER_LOG}"
  exit 0
fi

echo "[error] first run failed without OOM signature; check ${LOG_2048}" | tee -a "${MASTER_LOG}"
exit 1
